...
myOut = paste("trueMod","sim","simMod","map","integrated","sampleSize","TMBAIC","X0","k","Linf","a","a_mr", "sig_A","sig_A_mr", "sig_ki", "sig_kij", "sig_Linfi", "sig_x", "sig_L", "sig_bph", "beta1", "gr",sep="\t")
write.table(t(myOut), "simulationModelSampleSize.out", quote=FALSE, row.names = FALSE, col.names = FALSE)
      ...
      for(modi in trueModel){ #model 1=VB, 2=gomp, 3=log
        for(inti in 1:1){ #1 = Hum/MR, 2 = Hum, 3 = MR: Integrated or non-integrated data sets
          for(mapi in 1:1){ #which random effects do you want
            ...
            if(inti==1) myOut = c(trueModel,sim,modi,mapi,inti,sampleSize,TMBAIC(simOpt),rep$X0,rep$k,rep$Linf,rep$mu_a,rep$mu_a_mr,rep$sig_A,rep$sig_A_mr,rep$sig_ki, rep$sig_kij, rep$sig_Linfi, rep$sig_x, rep$sig_L, exp(rep$lnsig_bph), rep$beta1, max(obj$gr()))
            if(inti==2) myOut = c(trueModel,sim,modi,mapi,inti,sampleSize,TMBAIC(simOpt),rep$X0,rep$k,rep$Linf,rep$mu_a,NA,rep$sig_A,NA,rep$sig_ki, rep$sig_kij, rep$sig_Linfi, rep$sig_x, NA, NA, NA, max(obj$gr()))
            if(inti==3) myOut = c(trueModel,sim,modi,mapi,inti,sampleSize,TMBAIC(simOpt),rep$X0,rep$k,rep$Linf,NA,rep$mu_a_mr,NA,rep$sig_A_mr,rep$sig_ki, rep$sig_kij, rep$sig_Linfi, NA, rep$sig_L, NA, NA, max(obj$gr()))
            write.table(t(myOut), "simulationModelSampleSize.out", quote=FALSE, append = TRUE, row.names = FALSE, col.names = FALSE)
            print(paste(trueModel,sampleSize,sim,simOpt$max_gradient,simOpt$max_gradient<0.0001))
            }            
          }#end mapi
        }#end inti
      }#end modi
